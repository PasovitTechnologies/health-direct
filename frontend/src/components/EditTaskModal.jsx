...
import "../styles/EditTaskModal.css"; // Use EditTaskModal.css if separate, otherwise AddTaskModal.css

// Extend dayjs with the plugins
dayjs.extend(isSameOrAfter);
dayjs.extend(isSameOrBefore);

const EditTaskModal = ({ isOpen, onClose, onTaskUpdated, onTaskDeleted, task, selectedDate, refreshTasks }) => {
  ...
  // Update fields when modal opens or task changes
  useEffect(() => {
    if (isOpen) {
  ...
  const checkTimeOverlap = (newStart, newEnd) => {
    ...
    return existingTasks.some((existingTask) => {
      ...
      // Exclude the current task being edited to avoid self-conflict
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (task && task._id) {
        // Editing an existing task (PUT)
        method = "PUT";
        url = `http://localhost:5001/api/tasks/${task._id}`;
      } else {
      ...
      const action = task && task._id ? "updated" : "added";
      setMessage({ type: "success", text: `Task ${action} successfully!` });

      // Handle the response based on whether it's an update or add
      if (task && task._id) {
        onTaskUpdated(result.task); // Pass the updated task back to Tasks.jsx
      } else {
        onTaskUpdated(result.task); // For new tasks, pass the created task
      }
      ...
      // Close modal and refresh tasks after a short delay for user feedback
      setTimeout(() => {
        setMessage(null);
        onClose();
  ...
  const handleDelete = async () => {
    if (!task || !task._id) {
      setMessage({ type: "error", text: "No task to delete!" });
      return;
    }
    ...
    try {
      ...
      // Notify parent component to remove the task
      onTaskDeleted(task._id, task.date);
  ...
  return (
    <div className="modal-overlay" role="dialog" aria-labelledby="modal-title">
      <div className="modal-content">
        ...
        <h2 id="modal-title" className="modal-title">{task && task._id ? "Edit Task" : "Add New Task"}</h2>
        ...
        >
          ...
          ) : task && task._id ? "Save Task" : "Add Task"}
        </button>
